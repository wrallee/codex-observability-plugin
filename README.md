...
- **Checking dedup sidecars** — published turns are recorded next to the rollout as `<rollout>.jsonl.langfuse`. A matching `Stop` hook `turn_id` records the turn even before Codex writes `task_complete`, preventing a second upload on the next hook. Other unfinished turns wait for completion; if the hook has no matching turn ID, their export is delayed until a later hook sees the completion marker. Settings-only events do not create traces.
